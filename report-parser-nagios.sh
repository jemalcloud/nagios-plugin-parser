...
service_status=$(xmllint --html --xpath "//tr[td='Service Status']/following-sibling::tr[td='$service_name']/td[2]/text()" "$html_file" 2>/dev/null | xargs)

# Check if the service status is empty (service not found) or '----' (status not provided)
if [ -z "$service_status" ] || [ "$service_status" == "----" ]; then
    ...
    if [ "$service_status" == "error" ]; then
        echo "CRITICAL: Job count not completed or failed for interface '$service_name'"
        exit 2  # Nagios critical status
    elif [ "$service_status" == "available" ]; then
        echo "OK: Job completed for interface '$service_name'"
